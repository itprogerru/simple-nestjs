...
describe('UserRepository', () => {
  ...
  describe('signUp', () => {
    ...
    it('throws a conflic exeption as username already exists ', () => {
      save.mockRejectedValue({code: '23505'});
      expect(userRepository.signUp(mockCredentialsDto)).rejects.toThrow();
    });
    it('throws a InternalServerError exeption as username already exists ', () => {
  ...
  describe('validateUserPassword', () => {
    ...
    it('returns null as as user cannot be found', async () => {
      userRepository.findOne.mockResolvedValue(null);
      const result = await userRepository.validateUserPassword(mockCredentialsDto);
      expect(user.validatePassword).not.toHaveBeenCalled();
      expect(result).toBeNull();
    });
    it('returns null as password is invalidd', async () => {
      userRepository.findOne.mockResolvedValue(user);
  ...
  describe('hashPassword', () => {
    it('call bcrypt.has to generate a hash', async () => {
